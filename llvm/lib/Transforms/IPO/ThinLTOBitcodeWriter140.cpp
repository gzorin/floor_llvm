//===- ThinLTOBitcodeWriter140.cpp - Bitcode 14.0 writing pass for ThinLTO ===//
//
...
void splitAndWriteThinLTOBitcode140(
  ...
  if (ModuleId.empty()) {
    ...
    WriteBitcodeToFile140(M, OS, /*ShouldPreserveUseListOrder=*/false, &Index);

    if (ThinLinkOS)
      // We don't have a ThinLTO part, but still write the module to the
      // ThinLinkOS if requested so that the expected output file is produced.
      WriteBitcodeToFile140(M, *ThinLinkOS, /*ShouldPreserveUseListOrder=*/false,
                            &Index);
...
void writeThinLTOBitcode140(raw_ostream &OS, raw_ostream *ThinLinkOS,
  ...
  if (hasTypeMetadata(M)) {
    if (enableSplitLTOUnit(M))
      return splitAndWriteThinLTOBitcode140(OS, ThinLinkOS, AARGetter, M);
  ...
  // given OS.
  if (ThinLinkOS && Index)
    writeThinLinkBitcodeToFile140(M, *ThinLinkOS, *Index, ModHash);
}

class WriteThinLTOBitcode140 : public ModulePass {
...
public:
  ...
  WriteThinLTOBitcode140() : ModulePass(ID), OS(dbgs()), ThinLinkOS(nullptr) {
    initializeWriteThinLTOBitcode140Pass(*PassRegistry::getPassRegistry());
  }

  explicit WriteThinLTOBitcode140(raw_ostream &o, raw_ostream *ThinLinkOS)
      : ModulePass(ID), OS(o), ThinLinkOS(ThinLinkOS) {
    initializeWriteThinLTOBitcode140Pass(*PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override { return "ThinLTO Bitcode 14.0 Writer"; }

  bool runOnModule(Module &M) override {
    const ModuleSummaryIndex *Index =
        &(getAnalysis<ModuleSummaryIndexWrapperPass>().getIndex());
    writeThinLTOBitcode140(OS, ThinLinkOS, LegacyAARGetter(*this), M, Index);
    return true;
  }
...
char WriteThinLTOBitcode140::ID = 0;
INITIALIZE_PASS_BEGIN(WriteThinLTOBitcode140, "write-thinlto-bitcode140",
                      "Write ThinLTO 14.0 Bitcode", false, true)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(ModuleSummaryIndexWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(WriteThinLTOBitcode140, "write-thinlto-bitcode140",
                    "Write ThinLTO 14.0 Bitcode", false, true)

ModulePass *llvm::createWriteThinLTOBitcodePass140(raw_ostream &Str,
                                                   raw_ostream *ThinLinkOS) {
  return new WriteThinLTOBitcode140(Str, ThinLinkOS);
}

PreservedAnalyses
llvm::ThinLTOBitcodeWriterPass140::run(Module &M, ModuleAnalysisManager &AM) {
  FunctionAnalysisManager &FAM =
      AM.getResult<FunctionAnalysisManagerModuleProxy>(M).getManager();
  writeThinLTOBitcode140(OS, ThinLinkOS,
